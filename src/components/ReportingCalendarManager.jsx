...
const ReportingCalendarManager = ({
  ...
  const toggleStore = (storeKey) => {
    ...
    if (!row?.lifecycleReady) {
      notify("這間門市的營運期間設定尚未完成，暫時不能設定休店日", "error");
      return;
    }
  ...
  const confirmCredentialAction = async () => {
    ...
    try {
      ...
      else if (historicalMonths.length > 0) notify(`休店設定已更新；${historicalMonths.join("、")} 已排入歷史月報自動重整，完成後年度報表也會同步更新`, "success");
  ...
  return (
    <div className="space-y-6 w-full max-w-full min-w-0">
      <Card title="營運日曆 / 休店日管理">
        <div className="space-y-5">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <div className="rounded-2xl border border-[#EFE7DA] bg-[#FFFCF7] p-4"><div className="text-[11px] font-black text-[#A69C91]">資料版本</div><div className="mt-1 text-lg font-black text-[#4D4338]">{reportingCalendar.revision || 0}</div></div>
          ...
          <div className="rounded-2xl border border-sky-100 bg-sky-50/60 p-4 text-xs font-bold leading-6 text-sky-800">正式休店日會從「應回報日」排除，不需要建立 0 元日報。全品牌與指定店家都使用同一份正式休店設定；既有日報原始資料不會被修改。</div>
      ...
      </div><div className="space-y-4"><div className="rounded-2xl border border-[#EFE7DA] bg-[#FAF7F1] p-4"><div className="text-sm font-black text-[#4D4338]">設定預覽</div><div className="mt-4 space-y-3 text-xs"><div className="flex justify-between gap-3"><span className="font-bold text-[#A69C91]">品牌</span><span className="font-black text-[#4D4338]">{brandMeta.label}</span></div><div className="flex justify-between gap-3"><span className="font-bold text-[#A69C91]">範圍</span><span className="text-right font-black text-[#4D4338]">{scope === "brand" ? "全品牌" : `指定 ${selectedStoreKeys.length} 間店`}</span></div><div className="flex justify-between gap-3"><span className="font-bold text-[#A69C91]">日期</span><span className="text-right font-black text-[#4D4338]">{formatDateRange(selectedDates)}</span></div><div className="flex justify-between gap-3"><span className="font-bold text-[#A69C91]">原因</span><span className="text-right font-black text-[#4D4338]">{effectiveReason || "尚未填寫"}</span></div></div><div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50/70 p-3 text-[11px] font-bold leading-5 text-emerald-700">不會產生 0 元日報，也不會改變門市營運期間資料。若修改歷史月份，系統會自動安排月報重新整理。</div></div>
        <div className="rounded-2xl border border-[#EFE7DA] bg-white p-4"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0 text-[#B7863D]" /><div><div className="text-sm font-black text-[#4D4338]">最高管理者保護</div><div className="mt-1 text-[11px] font-bold leading-5 text-[#A69C91]">系統會重新確認登入身分、目前裝置信任狀態與最高管理者密碼。</div></div></div><div className={`mt-3 rounded-xl border px-3 py-2.5 text-xs font-black ${currentDeviceTrust?.status === "trusted" ? "border-emerald-100 bg-emerald-50 text-emerald-700" : "border-rose-100 bg-rose-50 text-rose-700"}`}>{currentDeviceTrust?.status === "trusted" ? "🛡 目前裝置已信任" : "⚠ 請改用已信任裝置"}</div><button type="button" onClick={openAddConfirmation} disabled={saving || currentDeviceTrust?.status !== "trusted"} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#E8C77A] bg-gradient-to-r from-[#FFF7DF] via-[#F7E8C6] to-[#EACB86] px-5 py-3 text-sm font-black text-[#5A4225] disabled:opacity-45">{saving ? <Loader2 size={17} className="animate-spin" /> : <CheckCircle2 size={17} />} 確認設定休店日</button></div></div></div></Card>}

      {activeMode === "month" && <Card title="月份檢視"><div className="space-y-4"><div className="max-w-sm"><div className="mb-1.5 text-xs font-black text-[#7C7063]">選擇要查看的月份日期</div><SmartDatePicker selectedDate={monthFocusDate} onDateSelect={setMonthFocusDate} /><div className="mt-1 text-[10px] font-bold text-[#A69C91]">選擇任一天，即可顯示該月份所有休店設定。</div></div><div className="rounded-2xl border border-[#EFE7DA] bg-[#FAF7F1] p-4"><div className="text-sm font-black text-[#4D4338]">{focusMonth || "—"}｜{monthHistory.length} 筆休店設定</div><div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">{monthHistory.map((event) => <div key={`${event.scope}-${event.id}`} className="rounded-xl border border-[#EFE7DA] bg-white p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black text-[#A69C91]">{formatDateRange(event.dates)}</div><div className="mt-1 text-sm font-black text-[#4D4338]">{event.reason || "休店"}</div></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-black ${event.scope === "brand" ? "border-amber-100 bg-amber-50 text-amber-700" : "border-sky-100 bg-sky-50 text-sky-700"}`}>{event.scope === "brand" ? "全品牌" : `${event.storeKeys?.length || 0} 店`}</span></div>{event.scope === "stores" && <div className="mt-3 flex flex-wrap gap-1.5">{(event.storeRows || []).map((row) => <span key={row.storeKey} className="rounded-full border border-[#EFE7DA] bg-[#FAF7F1] px-2 py-1 text-[10px] font-bold text-[#7C7063]">{row.canonicalStoreName}</span>)}</div>}</div>)}</div>{monthHistory.length === 0 && <div className="py-10 text-center text-xs font-bold text-[#A69C91]">這個月份目前沒有休店設定</div>}</div></div></Card>}
