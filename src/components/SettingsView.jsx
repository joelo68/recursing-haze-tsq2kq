...
import {
  ...
  UserCheck, UserX, Key, Calendar, DollarSign, Users
} from "lucide-react";
import { 
...
const SettingsView = () => {
  ...
  const availableTherapists = useMemo(() => {
    ...
    if (userRole === 'manager') { const myStores = managers[currentUser?.name] || []; return list.filter(t => myStores.includes(t.store)).sort((a,b) => a.store.localeCompare(b.store)); }
  ...
  const handleAddGlobalStore = async () => { 
    ...
    try { 
      ...
      const currentStores = managers[targetManager] || []; 
      await setDoc(doc(db, "artifacts", appId, "public", "data", "global_settings", "org_structure"), { 
        managers: { ...managers, [targetManager]: [...currentStores, newShop.name] } 
      }); 
  ...
  const handleDeleteGlobalStore = async (storeName, managerName) => { 
    if(!confirm(managerName === UNASSIGNED_KEY ? "確定永久刪除此店家？" : "確定將此店家移至『未分配』名單？")) return; 
    
    try { 
      const currentManagerStores = (managers[managerName] || []).filter(x => x !== storeName);
      let newManagers = { ...managers, [managerName]: currentManagerStores };

      if (managerName !== UNASSIGNED_KEY) {
        const unassignedStores = managers[UNASSIGNED_KEY] || [];
        if (!unassignedStores.includes(storeName)) {
          newManagers[UNASSIGNED_KEY] = [...unassignedStores, storeName];
        }
      }

      await setDoc(doc(db, "artifacts", appId, "public", "data", "global_settings", "org_structure"), { managers: newManagers }); 
      showToast(managerName === UNASSIGNED_KEY ? "已永久刪除" : "已移至未分配", "success"); 
    } catch(e) { 
      showToast("失敗", "error"); 
    } 
  };
  ...
  const handleSaveManagerStores = async (name) => { 
    try { 
      const originalStores = managers[name] || [];
      ...
      const currentUnassigned = managers[UNASSIGNED_KEY] || [];
      let nextUnassigned = [...currentUnassigned, ...removedStores];
      nextUnassigned = nextUnassigned.filter(s => !addedStores.includes(s));
      nextUnassigned = [...new Set(nextUnassigned)];

      const newStructure = { 
        ...managers, 
        [name]: newStores,
        [UNASSIGNED_KEY]: nextUnassigned
      };

      await setDoc(doc(db, "artifacts", appId, "public", "data", "global_settings", "org_structure"), { managers: newStructure }); 
  ...
  const availableStoresForManagerEdit = useMemo(() => { 
    return (managers[UNASSIGNED_KEY] || []).sort();
  }, [managers]);
  ...
  return (
    <ViewWrapper>
      <div className="space-y-6">
        ...
        {/* 6. 區長管理 (含未分配邏輯 + RWD 修正) */}
        {activeTab === "managers" && (
          <div className="space-y-6">
            <Card title="新增區長">
              <div className="flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1 w-full">
                  <label className="block text-xs font-bold text-stone-400 mb-1">區長姓名</label>
                  <input type="text" value={newManager.name} onChange={(e) => setNewManager({ ...newManager, name: e.target.value })} placeholder="例如: Jonas" className="w-full px-4 py-2 border-2 border-stone-100 rounded-xl outline-none focus:border-amber-400 font-bold" />
                </div>
                <div className="flex-1 w-full">
                  <label className="block text-xs font-bold text-stone-400 mb-1">預設密碼</label>
                  <input type="text" value={newManager.password} onChange={(e) => setNewManager({ ...newManager, password: e.target.value })} placeholder="設定密碼" className="w-full px-4 py-2 border-2 border-stone-100 rounded-xl outline-none focus:border-amber-400 font-bold" />
                </div>
                <button onClick={handleAddManager} className="w-full md:w-auto bg-stone-800 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-stone-700 shadow-sm flex items-center justify-center gap-2">
                  <Plus size={18} /> 新增區長
                </button>
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {Object.entries(managers).filter(([m]) => m !== UNASSIGNED_KEY).map(([managerName, stores]) => (
                <Card key={managerName} className="border border-stone-200">
                  <div className="flex flex-wrap justify-between items-start gap-3 mb-4">
                    <div>
                      <h3 className="text-lg font-bold text-stone-700 flex items-center gap-2">
                        <User size={20} className="text-amber-500" />{managerName} 區
                      </h3>
                      <p className="text-xs text-stone-400 mt-1 font-mono">密碼: {managerAuth[managerName] || "未設定"}</p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => { setEditingManager(managerName); setEditingManagerStores(stores); }} className="text-xs bg-stone-100 text-stone-600 px-3 py-1.5 rounded-lg hover:bg-stone-200 font-bold whitespace-nowrap">
                        編輯轄區
                      </button>
                      <button onClick={() => handleDeleteManager(managerName)} className="text-rose-400 hover:bg-rose-50 p-1.5 rounded-lg">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
                  ...
                  {editingManager === managerName ? (
                    <div className="mt-4 animate-in fade-in bg-stone-50 p-4 rounded-xl border border-stone-200">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {editingManagerStores.map((s) => (
                          <div key={s} className="group relative flex items-center">
                            <span className="px-3 py-1.5 bg-white border border-stone-200 rounded-lg text-xs font-bold text-stone-600 shadow-sm pr-7">{s}</span>
                            <button onClick={() => handleRemoveStoreFromEditing(s)} className="absolute right-1 p-1 text-stone-300 hover:text-rose-500 transition-colors">
                              <X size={12} />
                            </button>
                          </div>
                        ))}
                      ...
                      <div className="mb-4">
                        <label className="block text-xs font-bold text-stone-400 mb-1">新增未分配店家 (從未分配清單選擇)</label>
                        <div className="relative">
                          <select onChange={(e) => { handleAddStoreToEditing(e.target.value); e.target.value = ""; }} className="w-full px-4 py-2 border-2 border-stone-200 rounded-xl font-bold bg-white appearance-none text-stone-700">
                            <option value="">+ 點擊選擇店家</option>
                            {availableStoresForManagerEdit.filter((s) => !editingManagerStores.includes(s)).map((s) => (
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => setEditingManager(null)} className="px-3 py-1.5 text-xs font-bold text-stone-400 hover:text-stone-600">取消</button>
                        <button onClick={() => handleSaveManagerStores(managerName)} className="px-4 py-1.5 bg-stone-800 text-white text-xs font-bold rounded-lg hover:bg-stone-700 shadow-sm">儲存變更</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-wrap gap-2 mt-4">
                      {stores.map((s) => (
                        <span key={s} className="px-2.5 py-1 bg-stone-50 border border-stone-100 rounded-lg text-xs font-bold text-stone-600">{s}</span>
                      ))}
                    </div>
                  )}
                </Card>
