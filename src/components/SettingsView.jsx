...
const SettingsView = () => {
  ...
  useEffect(() => {
    const { annualAverageSettings: _legacyAnnualAverageSettings, ...runtimeFeatureFlags } = featureFlags || {};
  ...
  const handleSaveFeatureFlags = async () => {
    try {
      // Batch 7：annualAverageSettings 已退休為 legacy data。
      // 不刪除既有 Firestore 欄位，避免 destructive migration；新年度平均 authority
      // 完全由 Store Lifecycle + Reporting Completeness 決定。
      const { annualAverageSettings: _legacyAnnualAverageSettings, ...featureFlagsWithoutLegacyAnnualAverage } = localFeatureFlags || {};
      const payload = {
        ...featureFlagsWithoutLegacyAnnualAverage,
            ...
            <div className="space-y-5 w-full max-w-3xl">
              ...
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 rounded-xl bg-white p-2 text-emerald-600">
                    <Calendar size={18} />
                  </div>
                  <div className="min-w-0">
