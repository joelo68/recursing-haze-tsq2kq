  ...
  return (
    <ViewWrapper>
      <div className="space-y-8 pb-10">
        ...
        {/* ========================================================================= */}
        {/* 年度 KPI 總覽卡片 (改為清爽白金風格) */}
        {/* ========================================================================= */}
        <div className="bg-white rounded-2xl p-6 shadow-xl shadow-stone-200/50 border-2 border-amber-100 relative overflow-hidden">
          ...
          {/* 標題區 */}
          <div className="flex items-center gap-2 mb-6 border-b border-stone-100 pb-4 relative z-10">
            <div className="p-2 bg-amber-50 rounded-lg text-amber-500">
               <Target size={20} />
            </div>
            <div>
              <h2 className="text-lg font-bold tracking-tight text-stone-700">
                {selectedYear} 年度目標達成總覽
              </h2>
              <p className="text-xs text-stone-400 font-medium mt-0.5">
                Yearly Performance Overview
              </p>
            </div>
            <span className="text-xs bg-stone-100 text-stone-500 px-3 py-1 rounded-full ml-auto font-bold">
              權限範圍: 自動篩選
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            {/* 左邊：現金業績 */}
            <div className="relative group p-4 rounded-xl hover:bg-stone-50 transition-colors">
              <div className="flex justify-between items-end mb-3">
                <span className="text-stone-500 text-sm font-bold flex items-center gap-1">
                  <DollarSign size={16} className="text-amber-500" /> 年度現金業績
                </span>
                <span className="text-3xl font-extrabold text-stone-800 font-mono tracking-tight">
                  {fmtMoney(yearlyStats?.cashActual)}
                </span>
              </div>
              ...
              <div className="w-full bg-stone-100 rounded-full h-4 mb-2 overflow-hidden shadow-inner">
                {/* 進度條 (亮橘黃) */}
                <div 
                  className="bg-gradient-to-r from-amber-400 to-orange-400 h-full rounded-full transition-all duration-1000 ease-out shadow-[0_2px_10px_rgba(251,191,36,0.3)]" 
                  style={{ width: `${Math.min(yearlyStats?.cashAchievement || 0, 100)}%` }}
                ></div>
              </div>
              
              <div className="flex justify-between text-xs font-bold">
                <span className="text-stone-400">目標: {fmtMoney(yearlyStats?.cashTarget)}</span>
                <span className={`${yearlyStats?.cashAchievement >= 100 ? "text-emerald-500" : "text-amber-500"}`}>
                  達成率: {fmtNum(yearlyStats?.cashAchievement)}%
                </span>
              </div>
            </div>
            ...
            <div className="relative group p-4 rounded-xl hover:bg-stone-50 transition-colors">
              <div className="flex justify-between items-end mb-3">
                <span className="text-stone-500 text-sm font-bold flex items-center gap-1">
                  <Activity size={16} className="text-sky-500" /> 年度權責業績
                </span>
                <span className="text-3xl font-extrabold text-stone-800 font-mono tracking-tight">
                  {fmtMoney(yearlyStats?.accrualActual)}
                </span>
              </div>
              ...
              <div className="w-full bg-stone-100 rounded-full h-4 mb-2 overflow-hidden shadow-inner">
                {/* 進度條 (亮藍) */}
                <div 
                  className="bg-gradient-to-r from-sky-400 to-blue-400 h-full rounded-full transition-all duration-1000 ease-out shadow-[0_2px_10px_rgba(56,189,248,0.3)]" 
                  style={{ width: `${Math.min(yearlyStats?.accrualAchievement || 0, 100)}%` }}
                ></div>
              </div>
              
              <div className="flex justify-between text-xs font-bold">
                <span className="text-stone-400">目標: {fmtMoney(yearlyStats?.accrualTarget)}</span>
                <span className={`${yearlyStats?.accrualAchievement >= 100 ? "text-emerald-500" : "text-sky-500"}`}>
                  達成率: {fmtNum(yearlyStats?.accrualAchievement)}%
                </span>
              </div>
            </div>
          </div>
