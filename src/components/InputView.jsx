...
const StoreInputView = () => {
  ...
  const handleNumberChange = (key, value) => {
    ...
    const headcountFields = ['traffic', 'newCustomers', 'newCustomerClosings'];
    
    if (headcountFields.includes(key)) {
      if (rawValue.length > 2) {
        showToast("⚠️ 人數不可能超過 99 人，請確認是否誤填為業績金額！", "error");
        return;
      }
      setFormData(prev => ({ ...prev, [key]: rawValue }));
    } else {
  ...
  const renderField = (key) => {
    const isHeadcount = ['traffic', 'newCustomers', 'newCustomerClosings'].includes(key);
    
    return (
      <div key={key}>
        <label className={`block text-xs font-bold mb-1.5 ${key.includes('Refund') ? "text-rose-500" : "text-stone-500"}`}>
          {LABELS[key] || key}
          {isHeadcount && <span className="text-stone-300 font-normal ml-1">(Max 99)</span>}
        </label>
        <input 
          type="text" 
          value={formData[key]} 
          ...
          inputMode="numeric" 
          pattern="[0-9]*"
          maxLength={isHeadcount ? 2 : undefined}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-lg space-y-8">
        ...
        <div>
          <h4 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Layers size={14}/> 核心業績數據</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {renderField("cash")} {renderField("accrual")} {renderField("operationalAccrual")} {renderField("skincareSales")}
          </div>
        </div>

        <div>
          <h4 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-4 flex items-center gap-2"><Users size={14}/> 新客與客流</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {renderField("newCustomerSales")} {renderField("traffic")} {renderField("newCustomers")} {renderField("newCustomerClosings")}
          </div>
        </div>

        <div>
          <h4 className="text-xs font-bold text-rose-300 uppercase tracking-wider mb-4 flex items-center gap-2 border-b border-rose-50 pb-1 w-full"><TrendingDown size={14}/> 退費資訊 (若無則留白)</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {renderField("refund")} {renderField("skincareRefund")}
          </div>
        </div>
...
const TherapistInputView = () => {
  ...
  const { brandInfo } = useMemo(() => {
    ...
    if (currentBrand) {
      ...
      name = typeof currentBrand === 'object' ? (currentBrand.name || currentBrand.label || id) : id;
    }
  ...
  const handleNumberChange = (key, value) => {
    ...
    const headcountFields = ['serviceCount', 'newCustomerCount', 'oldCustomerCount', 'newCustomerClosings'];
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6 pb-20">
      <div className={`bg-gradient-to-br ${brandInfo.id.includes('anniu') ? 'from-teal-500 to-emerald-600' : brandInfo.id.includes('yibo') ? 'from-purple-500 to-indigo-600' : 'from-indigo-600 to-blue-600'} rounded-2xl p-6 text-white shadow-lg relative overflow-hidden`}>
        ...
        <div className="space-y-6">
          <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
             ...
             <div className="relative">
                ...
                <input type="text" value={formData.totalRevenue} readOnly className="w-full pl-10 pr-3 py-3 border-2 border-indigo-200 rounded-xl font-mono text-2xl font-bold text-indigo-700 bg-white outline-none" placeholder="0" />
             </div>
          </div>
