  ...
  const storeMetrics = useMemo(() => {
    ...
    return {
      ...
      dailyData: data.map((d) => ({
        ...
        accrual: d.accrual || 0, // Add Accrual
        traffic: d.traffic,
      })),
      budget,
    };
...
const InputView = () => {
  ...
  const saveReport = async (existingId = null) => {
    try {
      // 規則: 如果是1號，強制先儲存目標
      if (isFirstDay) {
        await handleUpdateTargets(true); // true = silent mode
      }

      // --- 強制轉換日期格式為 YYYY/MM/DD ---
      // 使用 toStandardDateFormat 確保儲存格式統一
  ...
  const handleSubmit = async (e) => {
    ...
    // --- 檢查是否有舊資料 (使用標準化後的日期比對) ---
    // 使用 toStandardDateFormat 確保格式統一 (YYYY/MM/DD)
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-bold text-stone-600 flex items-center gap-2">
                    ...
                    當月營運目標 ({new Date(inputDate).getMonth() + 1}月)
                    {isFirstDay && (
...
const HistoryView = () => {
  ...
  // Derived store list for filter dropdown
...
const SettingsView = () => {
  const {
    ...
    setTargets,
    showToast,
    managers, // 組織架構: { "區長名": ["店1", "店2"] }
    storeAccounts, // 店長帳號: [{ id, name, password, stores: [] }]
    managerAuth, // 區長密碼: { "區長名": "密碼" }
    userRole,
    appId, // 從 context 或 global 取得
    permissions,
  } = useContext(AppContext);

  // 內部狀態管理
  const [activeTab, setActiveTab] = useState("kpi"); // kpi, shops, stores, managers, permissions
  ...
  const [editingManager, setEditingManager] = useState(null); // 當前編輯的區長
  // 改為陣列狀態，方便增刪
  ...
  const handleAddStoreAccount = async () => {
    ...
    // Convert single string to array for storage
  ...
  // 計算目前未分配給任何「區長」的店家 (孤兒店家)
  // 來源：所有在店經理帳號中出現過的店家 - 目前組織架構中已分配的店家
  const orphanedStores = useMemo(() => {
    // 1. 取得所有已知店家 (從店經理帳號反推，這通常是最全的名單)
    const allKnownStores = [
      ...new Set(storeAccounts.flatMap((a) => a.stores || [])),
    ];

    // 2. 取得目前已分配給區長的店家 (編輯中的狀態不算，要算 global state)
    // 注意：如果是編輯模式，我們要排除「當前編輯區長」擁有的店家，
    // 但為了簡化，我們只列出「完全沒人管」的店家供新增。
    // 若要從別區搶過來，需先從別區刪除。
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          ...
                          setEditingManagerStores(stores); // 載入當前商店
                        }}
...
export default function App() {
  ...
  // Logging Function (Updated for Device Detection)
  ...
  // ... rest of the App component (data subscriptions, handlers, render) ...
  useEffect(() => {
    if (!user) return;
  ...
  const handleLogout = () => {
    ...
    // Clear draft on logout to prevent data leakage
  ...
  const visibleManagers = useMemo(() => {
    ...
    // NEW LOGIC FOR STORE ROLE: Filter managers structure to only contain user's stores
    if (userRole === ROLES.STORE.id && currentUser) {
      const myStores =
        currentUser.stores ||
        (currentUser.storeName ? [currentUser.storeName] : []);
      // Note: myStores usually holds short names based on settings logic
      const filteredManagers = {};
      Object.entries(managers).forEach(([mgr, stores]) => {
        // Find stores in this manager's list that belong to the user
  ...
  const analytics = useMemo(() => {
    ...
    const storeStats = storeList.map((s) => {
      ...
      return {
        ...
        // FIXED: Use Operational Accrual for Traffic ASP (Pure Technical)
        trafficASP:
          trafficTotal > 0
    ...
    return {
      ...
      // FIXED: Grand Total Avg Traffic ASP using Operational Accrual
      avgTrafficASP:
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div className="flex min-h-screen bg-[#F9F8F6] text-stone-600 font-sans selection:bg-stone-200 selection:text-stone-800 overflow-x-hidden">
        ...
        {/* Main Content with Mobile Top Nav */}
        <div
          ...
          <header className="h-20 bg-white/80 backdrop-blur-md border-b border-stone-200 sticky top-0 z-40 px-4 md:px-8 flex items-center justify-between shadow-sm shadow-stone-200/50 shrink-0 transition-all">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg font-bold text-stone-800 tracking-tight truncate md:hidden flex items-center gap-2">
                <Coffee size={20} className="text-amber-600" /> DRCYJ OPC Cloud
              </h1>
            </div>

            <div className="flex items-center gap-3 md:gap-5 flex-1 justify-end">
              {/* Quick Search - Pill Shape */}
